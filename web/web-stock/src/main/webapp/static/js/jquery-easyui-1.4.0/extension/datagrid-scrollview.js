...
//			var style = styleValue ? 'style="' + styleValue + '"' : '';
	...
	onBeforeRender: function(target){
		...
		function scrolling(){
			//若本地数据已全部显示，则直接返回，不在向服务器请求数据
			var personNum = state.originalRows.length;
			if(!personNum){
				return ;
			}
			var pageSize = opts.pageSize;
			if(personNum>pageSize){
				if(this.index+pageSize*2>=personNum){
					this.index = 0;
					return ;
				}
			}
			if (dc.body2.is(':empty')){
//				reload.call(this);    //若rows为空，此处不需要再次请求数据
			} else {
...
//					reload.call(this);    //若数据量较小，此处不需要再次请求数据
				} else if (top > 0){
					...
					if (this.rows.length&&opts.pageSize&&this.rows.length < opts.pageSize) {
						return ;
					}
						this.getRows.call(this, target, page, function(rows){
							if (!this.r2.length){
