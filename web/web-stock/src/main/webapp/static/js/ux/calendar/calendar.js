...
function drawCld(SY, SM) {
 rebuild();
 $.ajax({
				url : Feng.ctxPath + "holidayController/dataList.do",
				type : "post",
				//async: false,
				dataType : "json",
				data:{
				...
				success : function(data) {
					if (data.success==true){	
                     ...
                     for (i = 0; i < 42; i++) {
						...
						var hstr = data.vacations;
						
						if(hstr.indexOf(nowDays)>-1){
						   
							 $("#GD" + i).css("background","#FBBB67");
							 //$("#GD" + i).addStyle('background');
							// $("#GD" + i).style.background='#FBBB67';
						}
...
function rebuild(){
   ...
   $('#huiyi8 td').each(function(){
                $(this).css("background","");//还原所有td的颜色
