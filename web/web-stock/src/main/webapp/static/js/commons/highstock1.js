    ...
    */
    ...
    var DATA_GROUPING = 'dataGrouping',
        ...
        approximations = {
            ...
            close: function (arr) {
                return arr.length ? arr[arr.length - 2] : (arr.hasNulls ? null : UNDEFINED);
            },
			increase: function (arr) {
                return arr.length ? arr[arr.length - 1] : null;
            },
            // ohlc and range are special cases where a multidimensional array is input and an array is output
            ohlc: function (open, high, low, close,increase) {
                open = approximations.open(open);
                high = approximations.high(high);
                low = approximations.low(low);
                close = approximations.close(close);
				increase = approximations.increase(increase);

                if (isNumber(open) || isNumber(high) || isNumber(low) || isNumber(close)|| isNumber(increase)) {
                    return [open, high, low, close,increase];
                }
    ...
    var OHLCSeries = extendClass(seriesTypes.column, {
        type: 'ohlc',
        pointArrayMap: ['open', 'high', 'low', 'close','increase'], // array point configs are mapped to this
        toYData: function (point) { // return a plain array for speedy calculation
            return [point.open, point.high, point.low, point.close,point.increase];
        },
        pointValKey: 'high',
